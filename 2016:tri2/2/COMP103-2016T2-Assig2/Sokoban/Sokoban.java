...
public class Sokoban {
    ...
    //set three stacks for redo, direction and moveBox.
    private Stack<String>redo=new Stack<String>();
    private Stack<String>direction= new Stack<String>();
    private Stack<Boolean>moveBox=new Stack<Boolean>();
    ...
    public void undo(){
        if(!direction.isEmpty()&&!moveBox.isEmpty()&&moveBox.peek()){
            pull(oppositeDirection(direction.peek()));
            redo.push(direction.peek());
            direction.pop();
            moveBox.pop();
        }else if(!direction.isEmpty()&&!moveBox.isEmpty()){
            move(oppositeDirection(direction.peek()));
            redo.push(direction.peek());
            direction.pop(); 
            moveBox.pop();
        }else{
         UI.println("nothing can be undo");
        }
    }
    public void redo(){
        if(!redo.isEmpty()){
            ...
            UI.println("nothing can be redo");        
        }        
    }
