...
public class BiHashMap<K, V> implements Map<K, V> {
     ...
     * 
     *(A null return can also indicate that 
     *the map previously associated null with key).
	 */
	@Override
	//FIXED: CHANGE object into generic type V and K
	public V put(K key, V value) {
		// first, update the value associated with this key
		V previousValue = keyToValues.get(key);
		// recursion
		previousValue = keyToValues.put(key, value);
		
		
		// now, update the set of keys associated with this value
		//get a set of keys relevant to the value
		HashSet<K> keys = valueToKeys.get(value);
		// probably, the value is mapping to null (key)
		if (keys == null) {
			keys = new HashSet<>();
			// A empty set mapping to the value,not a null
			valueToKeys.put(value, keys);
		}
		keys.add(key);
		//update the set of keys of the valueToKeys
		//remove the "old key" which is associated with input value
		if(previousValue != null && !previousValue.equals(value)){
			HashSet<K> previousKeys = valueToKeys.get(previousValue);
			previousKeys.remove(key);
		}
		
		// finally, return the original mapping 
		// in order to ad here to Map interface.
